...
const ClassDiary = () => {
    ...
    const classdiarylook = (date, thisid) =>{
      navigate(`/classDiarylook`, { state: { date: `${date}`, id: `${thisid}`} });
    };
...
return (
  <ClassroomWrapper>
    <CoursesBox>
    <h2 style={{color:"#20C075", fontWeight:"bold"}}><strong class="fontMedium"><strong style={{color:"black"}}>{user} 님의</strong> '수업 일기장'</strong></h2>
    <Underline/>
      ...
      <DateList>
        {currentDates.map((date, index) => (
            <DateItem key={index}>
                ...
                <Button onClick={() => classdiarylook(date, id[index])}>일기 보기</Button> 
            </DateItem>
        ))}
      </DateList>
